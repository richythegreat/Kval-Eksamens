<x-guest-layout>
    <div class="min-h-screen grid place-items-center px-4">
        <div class="w-full max-w-md rounded-2xl bg-white/5 dark:bg-white/5 ring-1 ring-gray-200 dark:ring-white/10 backdrop-blur-xl p-6">
            <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Pierakstīšanās</h1>
            @if (session('status'))
            ...
            <form method="POST" action="{{ route('login') }}" class="mt-6 space-y-4">
                ...
                <div class="flex items-center justify-between">
                    ...
                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}" class="text-sm text-gray-700 hover:underline dark:text-white/80">Aizmirsī?</a>
                    @endif
                </div>
            ...
            <p class="mt-6 text-center text-sm text-gray-600 dark:text-white/70">
                Vai jūms vēl nav konts?
                <a href="{{ route('register') }}" class="underline">Izveidot kontu</a>
            </p>
            @endif
        </div>
    </div>

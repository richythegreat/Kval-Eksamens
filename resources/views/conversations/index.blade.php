...
<x-app-layout>
    <div class="max-w-4xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
        <h1 class="text-2xl sm:text-3xl font-semibold tracking-tight text-white mb-6">Saziņa</h1>

        @if ($conversations->isEmpty())
            <div class="rounded-2xl bg-white/5 ring-1 ring-white/10 backdrop-blur-xl p-10 text-center text-white/70">
                Jūs neesat uzsācis sarunu.
            </div>
        @else
            <div class="space-y-3">
                            ...
                            <div class="flex items-start gap-3 min-w-0">
                                ...
                                <div class="min-w-0">
                                    <div class="font-medium text-white truncate">
                                        {{ $otherUser?->name ?? 'Nezinams lietotājs' }}
                                    </div>
                                    <div class="text-sm text-white/70 truncate">
                                        {{ $lastMessage?->body ?? 'Nav ziņu' }}
                                    </div>
                                </div>
                            </div>
